...
export function SoulFoodOrderModal({
  isOpen,
  onClose,
}: SoulFoodOrderModalProps) {
      ...
      <DialogContent className="sm:max-w-[425px] bg-zinc-900 text-zinc-300">
        ...
        <ScrollArea className="mt-4 max-h-[60vh] pr-4">
          <div className="space-y-4">
            {cartItems.map((item) => (
              <div
                key={`${item.id}-${item.selectedIngredients.join("-")}`}
                className="flex items-center space-x-4 bg-zinc-800 p-3 rounded-lg"
              >
        ...
        <DialogFooter className="mt-6 block">
          ...
          <Button
            ...
            className="w-full bg-amber-600 hover:bg-amber-700 text-white text-lg font-semibold py-2"
          >
